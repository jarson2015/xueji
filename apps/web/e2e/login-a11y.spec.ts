...
test.describe('学迹登录码可读名称', () => {
  test('码区与 ⌫/清空有 aria-label', async ({ page }) => {
    await clearSession(page)
    await page.getByRole('button', { name: '学生进入' }).click()
    await expect(page.getByRole('group', { name: '6 位登录码' })).toBeVisible()
    await expect(page.getByRole('button', { name: '删除一位' })).toBeVisible()
    await expect(page.getByRole('button', { name: '清空登录码' })).toBeVisible()
