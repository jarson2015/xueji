...
test.describe('学迹休息日 / 删任务 SoftPrompt', () => {
  test('学生任务档案：休息日·拿到今日 SoftPrompt', async ({ page }) => {
    ...
    await expect(
      page.getByRole('button', { name: '休息日·拿到今日' }),
    ).toBeVisible()
    await page.getByRole('button', { name: '休息日·拿到今日' }).click()

    const dialog = page.getByRole('dialog')
    await expect(dialog).toBeVisible()
    await expect(page.getByRole('heading', { name: '休息日' })).toBeVisible()
    await expect(dialog.locator('.sp-msg')).toContainText(/先不催/)
    await expect(dialog.getByRole('button', { name: '拿到今日' })).toBeVisible()
    await dialog.getByRole('button', { name: '先休息' }).click()
    await expect(dialog).toHaveCount(0)
  })

  test('家长任务：单条删除 SoftPrompt', async ({ page }) => {
    await clearSession(page)
    ...
    await expect(dialog.locator('.sp-msg')).toContainText(/立刻看不到/)
