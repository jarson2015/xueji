...
async function enterAsFirstStudent(page: Page) {
  ...
  const enter = page.getByRole('button', { name: '帮孩子进入今日' }).first()
  await expect(enter).toBeVisible({ timeout: 15_000 })
  await enter.click()
  await expect(page).toHaveURL(/\/student/, { timeout: 20_000 })
  await expect(page.getByText(/家长代登中/)).toBeVisible()
}

test.describe('学迹代登退出', () => {
  test('正常退出回到家长学生管理', async ({ page }) => {
    await loginAsParent(page)
    await enterAsFirstStudent(page)
    await page.getByRole('button', { name: '退出代登' }).click()
    await expect(page).toHaveURL(/\/parent\/students/, { timeout: 15_000 })
    await expect(page.getByText(/家长代登中/)).toHaveCount(0)
  })

  test('备份损坏则回登录，不假进家长页', async ({ page }) => {
    await loginAsParent(page)
    await enterAsFirstStudent(page)
    await page.evaluate(() => {
