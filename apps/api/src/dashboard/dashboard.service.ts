...
export class DashboardService {
  ...
  async progress(parentId: number) {
    ...
    // 对齐 myTasksForStudents：无 steps、排除 day_archived、每孩封顶
    const [tasksByStudent, allPending] = await Promise.all([
    ...
    for (const s of students) {
      const items = (tasksByStudent.get(s.id) || []).map((n) => ({
        ...n,
        stuckStep: null,
      }));
      result.push({
        ...
        completionRate: items.length
          ? Math.round(
              (items.filter((i) => i.progressPercent >= 100).length /
                items.length) *
                100,
            )
          : 0,
  ...
  private async buildToday(studentId: number, opts: { lite: boolean }) {
    ...
    // 周汇总结算仅走 scheduler / 报告显式入口，今日读路径纯读
    const digestSettlement: { points: number; settled: number } | null = null;
  ...
  /**
   * Legacy thin summary — UI uses /dashboard/monitor.
   * PERF P6: project from monitor(lite) instead of a second heavy path.
   */
  async summary(parentId: number) {
    const [m, settings] = await Promise.all([
      this.monitor(parentId, { lite: true }),
      this.family.getOrCreate(parentId),
    ]);
    return {
      date: m.date,
      headline: m.headline ?? m.family?.headline,
      totalDue: m.totalDue ?? m.family?.totalDue ?? 0,
      totalDone: m.totalDone ?? m.family?.totalDone ?? 0,
      children: (m.children || []).map((c: any) => ({
        studentId: c.studentId,
        name: c.name,
        pointsBalance: c.pointsBalance ?? c.stats?.pointsBalance,
        due: c.due ?? c.stats?.due ?? 0,
        done: c.done ?? c.stats?.done ?? 0,
        unfinishedTitles: c.unfinishedTitles || [],
        isRestDay: !!c.isRestDay,
      })),
      pendingConfirms: m.pendingConfirms || [],
      pactAlert: m.pactAlert ?? null,
      rewardFadeHint: m.rewardFadeHint ?? null,
      fairnessHint: m.fairnessHint ?? null,
      overdueRedeemHint: m.overdueRedeemHint ?? null,
      giftFairnessHint: m.giftFairnessHint ?? null,
      parentOverloadHint: m.parentOverloadHint ?? null,
  ...
  async monitor(
    ...
    const [restConfigs, tasksByStudent, plansByStudent, streakByStudent, pendingRows, todayCheckins, todayRedeems] =
      studentIds.length
        ? await Promise.all([
            ...
            // 待确认：先窄字段建映射；详情列表仅再取前 20 条带 join
            this.checkins.find({
              where: {
                studentId: In(studentIds),
                confirmStatus: ConfirmStatus.PENDING,
    ...
    const pendingTop = (pendingRows as CheckIn[]).slice(0, 20);
    const pendingDetailById = new Map<number, CheckIn>();
    if (pendingTop.length) {
      const detail = await this.checkins.find({
        where: { id: In(pendingTop.map((c) => c.id)) },
        relations: ['student', 'task'],
      });
      for (const d of detail) pendingDetailById.set(d.id, d);
    }
    const pendingList = pendingTop.map((c) => {
      const full = pendingDetailById.get(c.id);
      return {
        id: c.id,
        studentId: c.studentId,
        studentName: full?.student?.name,
        taskTitle: full?.task?.title || '计划完成',
        note: c.note,
        imageUrl: c.imageUrl,
    ...
    // One 14d checkins load for fairness + coach; defer assigns only need skipDate
    const deferSinceKey = (() => {
      const d = new Date();
    ...
    const [
      ...
      this.family.getOrCreate(parentId),
      studentIds.length
        ? this.loadInsightCheckins(studentIds, 14)
        : Promise.resolve([] as CheckIn[]),
      ...
      studentIds.length
        ? this.assigns.find({
            where: {
              studentId: In(studentIds),
              skipDate: MoreThanOrEqual(deferSinceKey),
            },
            select: ['id', 'studentId', 'skipDate'],
          })
        : Promise.resolve([] as TaskAssign[]),
    ]);
    const fairnessHint = this.buildSharedFairnessHintFromRows(
      studentIds,
      students,
      insightCheckins,
    );
    const coachInsights = this.computeCoachInsightsFromRows(
      insightCheckins,
      assignRows,
      taskStats,
    );
  ...
  /** Batch checkins for monitor fairness/coach — narrow columns. */
  private async loadInsightCheckins(studentIds: number[], days: number) {
    const since = new Date();
  ...
  private async buildSharedFairnessHint(
    ...
    if (studentIds.length < 2) return null;
    const rows = await this.loadInsightCheckins(studentIds, 14);
    return this.buildSharedFairnessHintFromRows(studentIds, students, rows);
  }

  private buildSharedFairnessHintFromRows(
    studentIds: number[],
    students: User[],
    rows: CheckIn[],
  ) {
    ...
    for (const c of rows) {
      if (new Date(c.createdAt).getTime() < sinceMs) continue;
  ...
  private async computeCoachInsights(
    ...
    const [checkinRows, assigns, stats] = await Promise.all([
      this.loadInsightCheckins(studentIds, 7),
      this.assigns.find({ where: { studentId: In(studentIds) } }),
      taskStats
        ? Promise.resolve(taskStats)
        : this.tasks.taskActivityStatsForParent(parentId),
    ]);
    return this.computeCoachInsightsFromRows(checkinRows, assigns, stats);
  }

  private computeCoachInsightsFromRows(
    checkinRows: CheckIn[],
    assigns: TaskAssign[],
    stats: { activeTaskCount: number; confirmTaskCount: number },
  ) {
    const since = new Date();
