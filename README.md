...
- **Docker 项目管理（推荐）**：[`docs/DEPLOY_DOCKER_PROJECT.md`](./docs/DEPLOY_DOCKER_PROJECT.md) —编排 `docker-compose.fnos.yml`，只跑api+web，连已有 MySQL  
- 补充：[`docs/DEPLOY_FNOS.md`](./docs/DEPLOY_FNOS.md)、[`docs/DEPLOY_FNOS_1PANEL.md`](./docs/DEPLOY_FNOS_1PANEL.md)  
- **无Docker**：[`docs/DEPLOY_FNOS_NATIVE.md`](./docs/DEPLOY_FNOS_NATIVE.md) / `scripts/pack-fnos-native.ps1` 安装包

## 本地开发（无Docker 时可生SQLite）

本机未安装Docker 时，后端默认 `DB_TYPE=sqlite`，数据文件在 `apps/api/data/study.sqlite`。
...
本Docker 时将 `.env` 中`DB_TYPE=mysql`，并 `docker compose up -d mysql`，或直接 `docker compose up -d --build`。
...
2. **数据**：`DB_SYNCHRONIZE=false` + `DB_MIGRATIONS_RUN=true`；*关闭** `RUN_SEED_ON_START`；配置MySQL 备份；勿保留演示账号于生产库。
3. **演示面*：生产默认`ENABLE_DEMO_HINTS=false`（`/api/auth/demo-hints` 不返回密码登录码；登录页无演示区）。
...
7. **首次体验**：新家庭走完家长 **4 步* / 学生 **2 步*引导；先只开「任务打卡+愿望」，进阶能力在「家庭」里。
...
| 学生小明 | 登录码**10293847**（或账号 student1）| demo1234 |
| 学生小红 | 登录码**20384756**（或账号 student2）| demo1234 |

登录页可切换「家长登录/ 学生进入」：学生生6 位数字码即可进入今日页。 
...
| POST | /api/checkins/:id/confirm | 确认（可带note）|
| CRUD | /api/plans | 学习计划 |
| GET | /api/dashboard/progress | 家长进度详情 |
| GET | /api/dashboard/summary | 家长今日摘要 |
| GET/PUT | /api/family/settings | 家庭休息日/ 补上进度 / 积分策略 / 分龄 / 零花钱约家|
...
| GET | /api/allowance/me | 学生零花钱摘要（未开开关返回enabled:false）|
...
| GET | /api/push/vapid-public-key | Web Push 公钥（未配置分disabled）|
| POST | /api/push/subscribe | 学生订阅推送|
| DELETE | /api/push/subscribe | 取消订阅 |
| WS | /ws | 实时事件（JWT）|

Web Push 仅用于家长「轻轻提醒」（nudge）；需 HTTPS 成localhost，iOS 限制较多。番茄钟为今日页纯前端计时，不写后端。家长发布指派任务时学生端会收到站内「新任务」消息条：点击跳转关联任务；60 秒未点则软刷新一次列表（避免频繁全量拉取）。
...
微信系统通知、勋章、原生App；Push 目前仅覆盖家长「轻轻提醒」。

本迭代已完成：家庭休息日、家长温和提醒（站内 + Web Push nudge）、去对抗文案与品牌跨端美化、双家长邀请码共享孩子、生产MySQL 迁移开关、加厚周报、学生「下一件」番茄钟（纯前端）、打卡确认点赞短评、金手指愿望（积分兑换免一次家务）、任务时效过期、补上进度（折扣积分）、每日未完成温和提示、*首次全流程引导*、*信息架构收敛**、*家庭公约**、*共家长重要操作通知**、*奖励褪除/分龄/反思题**、*情商与生活任务模板*、*领域单测与健康检查扩展*、*零花钱账本（与积分隔离，默认关；演示小明已开）*。

安全加固：防重复打卡刷积分、生产JWT 强密钥校验、登录限流、上传图片白名单、`imageUrl` 仅允许本端`/uploads/`。
